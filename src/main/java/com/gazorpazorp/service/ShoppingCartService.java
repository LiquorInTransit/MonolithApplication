...
public class ShoppingCartService {
	...
	public Integer getCount() throws Exception {
		...
		return cart.getLineItems().stream().mapToInt(LineItemDto::getQty).sum();
	}
	...
	public ShoppingCart aggregateCartEvents(Long customerId) throws Exception {
		...
		if (!shoppingCart.getProductMap().isEmpty())
			products = new HashSet<>(productService.getProductsById(shoppingCart.getProductMap().entrySet().stream().map(e ->e.getKey().toString()).collect(Collectors.joining(","))));
	...
	public CheckoutResult checkout(Long quoteId) throws Exception {
		...
		if (currentCart != null) {
			...
			List<Inventory> inventory = inventoryService.getInventoryForProductIds(currentCart.getLineItems().stream().map(LineItemDto::getProductId).map(Object::toString).collect(Collectors.joining(",")), quoteId);
			...
			if (!inventory.isEmpty()) {
				...
				if (checkAvailableInventory(checkoutResult, currentCart, inventoryItems)) {
					...
					try {
						ResponseEntity<Order> orderEntity = orderService.createOrder(currentCart.getLineItems().stream().map(item->new LineItem(item.getProduct(),item.getQty())).collect(Collectors.toList()), quoteId, this.customer.getId());
	...
	public boolean checkAvailableInventory(CheckoutResult checkoutResult, ShoppingCart currentCart, Map<Long, Integer> inventoryItems) {
		...
		try {
			List<LineItemDto> inventoryNotAvailable = currentCart.getLineItems().stream().filter(item -> inventoryItems.get(item.getProductId()) - item.getQty() < 0).collect(Collectors.toList());
			
			if (inventoryNotAvailable.size() > 0) {
				String productIdList = inventoryNotAvailable.stream().map(LineItemDto::getProduct).map(Product::getName).collect(Collectors.joining(", "));
